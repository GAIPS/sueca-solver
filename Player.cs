...
namespace SuecaSolver
{
	public abstract class Player
	{
		...
		public int HighestRankForSuit(Suit leadSuit, Suit trump)
		{
			...
			{
				if (!card.HasBeenPlayed && card.Suit == leadSuit && (int) card.Rank > highestFromLeadSuit)
				{
					highestFromLeadSuit = (int) card.Rank;
				}
				else if (!card.HasBeenPlayed && card.Suit == trump && (int) card.Rank > highestTrump)
				{
					highestTrump = (int) card.Rank;
				}
			}
